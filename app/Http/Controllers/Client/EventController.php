...
class EventController extends Controller
{
    ...
    public function show(Request $request, Event $event)
    {
        ...
        // Si c'est une requête API (JSON)
        if ($request->wantsJson() || $request->is('api/*')) {
            // Enrichir les données de l'événement comme dans la méthode index
            $enrichedEvent = [
                'id' => $event->id,
                'organizer_id' => $event->organizer_id,
                'category_id' => $event->category_id,
            ...
            if ($event->venue) {
                $enrichedEvent['venue_name'] = $event->venue->name;
                $enrichedEvent['venue_city'] = $event->venue->city;
                $enrichedEvent['venue_address'] = $event->venue->address;
            }
