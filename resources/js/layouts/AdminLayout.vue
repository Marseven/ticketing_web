<template>
  <div class="min-h-screen bg-gray-50">
    ...
    <div class="fixed inset-y-0 left-0 z-50 w-64 bg-white shadow-lg transform transition-transform duration-300 ease-in-out lg:translate-x-0 flex flex-col"
      ...
      <!-- Logo (fixe) -->
      <div class="flex items-center justify-center h-16 px-4 flex-shrink-0" style="background-color: #272d63;">
      ...
      <nav class="flex-1 overflow-y-auto mt-6 px-4 pb-6">
        <div class="space-y-2">
          ...
          <router-link to="/admin/tickets"
                       class="flex items-center px-4 py-3 text-gray-700 rounded-lg transition-colors duration-200"
                       :class="getMenuItemClass('/admin/tickets')"
                       :style="getMenuItemStyle('/admin/tickets')"
                       @mouseover="handleMenuHover($event, '/admin/tickets')"
                       @mouseleave="handleMenuLeave($event, '/admin/tickets')">
            <svg class="w-5 h-5 mr-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
