<template>
  <div class="ticket-component bg-white rounded-primea-xl overflow-hidden font-primea max-w-sm shadow-primea-lg">
    ...
    <div class="ticket-actions p-4 bg-white flex flex-col sm:flex-row gap-3">
      <button
        @click="$emit('view', ticket)"
        class="flex-1 bg-primea-blue text-primea-white py-3 px-4 rounded-primea font-bold text-sm hover:bg-primea-blue/80 transition-all duration-200 flex items-center justify-center gap-2 shadow-lg hover:shadow-xl min-h-[48px]"
      >
        ...
        <span class="whitespace-nowrap">Voir le ticket</span>
      </button>

      <button
        @click="$emit('download', ticket)"
        class="flex-1 bg-primea-yellow text-primea-blue py-3 px-4 rounded-primea font-bold text-sm hover:bg-primea-yellow/80 transition-all duration-200 flex items-center justify-center gap-2 shadow-lg hover:shadow-xl min-h-[48px]"
      >
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-5 h-5 flex-shrink-0">
          <path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3" />
        </svg>
        <span class="whitespace-nowrap">Télécharger PDF</span>
      </button>
    </div>
  </div>
</template>
