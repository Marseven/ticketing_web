<template>
  <article v-if="event && event.title" class="event-card-modern group cursor-pointer" @click="goToEvent">
    ...
    <div class="event-image">
      ...
      <div class="absolute bottom-4 left-4 right-4 flex items-end justify-between">
        ...
        <button
          ...
          class="ticket-btn-animate bg-yellow-500 text-blue-950 px-4 py-2 rounded-lg text-sm font-bold hover:bg-yellow-400 transition-all flex items-center gap-1 shadow-lg hover:shadow-xl"
        >
    ...
    <div class="p-6">
      ...
      <!-- Organisateur -->
      <div v-if="organizerName" class="flex items-center gap-1 mb-3 text-sm text-gray-600">
...
export default {
  ...
  setup(props) {
    ...
    const organizerName = computed(() => {
      // Plusieurs sources possibles pour le nom de l'organisateur
      if (props.event.organizer?.name) {
        return props.event.organizer.name
      }
      if (props.event.organizer_name) {
        return props.event.organizer_name
      }
      return null
    })
